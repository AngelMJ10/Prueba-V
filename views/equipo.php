      ...
      <div class="modal-body">
        <form>
          <div class="row mb-3">
            ...
            <div class="col-md-4">
              <div class="form-floating mb-3">
                <input type="numero" class="form-control" id="integrante-editar" placeholder="Integrantes" name="integrante">
                <label for="integrante" class="form-label">Integrantes</label>
              </div>
            </div>
        ...
        <div class="table-responsive mt-3" id="tabla-equipo">
          <table class="table table-hover"> 
...
<div class="modal fade" id="modal-empleado" tabindex="-1" aria-labelledby="modalEditarLabel" aria-hidden="true">
  <div class="modal-dialog modal-fullscreen">
    <div class="modal-content">
      <div class="modal-header text-light" style='background-color: #005478;'>
        <h1 class="modal-title fs-5" id="modalEditarLabel">Registrar Integrantes</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form>
          <div class="row mb-3">
            <div class="col-md-4">
              ...
              <div class="form-floating mb-3">
                  <input type="number" class="form-control" id="id-integrante" placeholder="Id Integrante" name="nombre">
                  <label for="integrante" class="form-label">Id Integrante</label>
              </div>
              <div class="form-floating mb-3">
                  <input type="text" class="form-control" id="name-equipo" placeholder="Id Integrante" name="nombre">
                  <label for="integrante" class="form-label">Nombre</label>
              </div>
              <div class="form-floating mb-3">
                  <input type="number" class="form-control" id="no-integrante" disabled placeholder="Id Integrante" name="nombre">
                  <label for="integrante" class="form-label">ID</label>
              </div>
        ...
        <div class="table-responsive mt-3" id="tabla-empleados">
          <table class="table table-hover"> 
    ...
    <script>
        ...
        function registrarIntegrante(){
          ...
          let id = $(this).data("id");
          if (confirm("¿Está seguro de registrar este integrante?")) {
            let datos ={
              'op'                    : 'registrarIntegrante',
              'idUsuario'             : idUsuario,
              'idIntegrante'          : $("#id-integrante").val(),
              'id'                    : id
            }
        ...
        $("#myTabContent").on("click", ".registrar",function(){
          let id = $(this).data("id");
          $.ajax({
              url: '../controllers/equipo.php',
              type: 'POST',
              data: 'op=listarEmpleados',
              success: function(result){
            ...
            $.ajax({
              ...
              success: function(result){
                let registro = JSON.parse(result);
                  $("#modal-empleado").modal('show');
                  if (registro) {
                    $("#name-equipo").val(registro['nombre']);
                    $("#no-integrante").val(registro['id']);
                  }
              }
            });
        ...
        $("#modal-empleado").on("click", ".registrar-empleado",function(){
          let idusuario = $(this).data("idusuario");
          let id = $("#no-integrante").val();
          if (confirm("¿Está seguro de registrar este integrante?")) {
            let datos ={
              'op'                    : 'registrarIntegrante',
              'idUsuario'             : idusuario,
        ...
        $("#myTabContent").on("click", ".editar-btn",function(){
          ...
          $.ajax({
            url: '../controllers/equipo.php',
            type: 'POST',
            data: 'op=obtenerIntegrantesPorEstado&id=' + id,
            success: function(result){
              let registro = JSON.parse(result);
              fila = '';
              if (registro) {
                $("#nombre-editar").val(registro['nombre']);
                let cantidad  = Object.keys(registro.integrantes).length
                $("#integrante-editar").val(cantidad);
                $("#estado-editar").val(registro['estado']);
                $("#id-editar").val(registro['id']);
                $("#modalEditar").modal('show');

                // Datos de los integrantes
                  if (registro && registro.integrantes) {
                  let integrantes = registro.integrantes;
                  let tablaIntegrantes = '';
                    // Declara una variable key que representará cada clave del objeto integrantes
                    for (let key in integrantes) {
                      let integrante = integrantes[key];
                      let estado = parseInt(integrante.datosEmpleado.estado) === 1 ? 'Activo' : 'Inactivo';
                      let disponible = parseInt(integrante.disponible) === 1 ? 'Disponible' : 'Trabajo asignado';

                      tablaIntegrantes += `
                        <tr>
                          <td class='p-3' data-label='ID'>${integrante.datosEmpleado.id}</td>
                          <td class='p-3' data-label='Nombre'>${integrante.datosEmpleado.nombre}</td>
                          <td class='p-3' data-label='Apellidos'>${integrante.datosEmpleado.apellidos}</td>
                          <td class='p-3' data-label='Tipo de Documento'>${integrante.datosEmpleado.nombreTipoDocumento}</td>
                          <td class='p-3' data-label='Documento'>${integrante.datosEmpleado.documento}</td>
                          <td class='p-3' data-label='Teléfono'>${integrante.datosEmpleado.telefono}</td>
                          <td class='p-3' data-label='Disponible'><span class='badge rounded-pill' style='background-color: #005478'>${disponible}</span></td>
                          <td class='p-3' data-label='Estado'><span class='badge rounded-pill' style='background-color: #005478'>${estado}</span></td>
                          <td><button type='button' id='editar-boton' data-id='${registro.id}' class='btn btn-outline-danger btn-sm editar-btn'><i class='fa-solid fa-trash'></i></button></td>
                        </tr>
                      `;
                    }
                  $("#tabla-equipo tbody").html(tablaIntegrantes);
                }
              }
            }
          });
        ...
        $("#registrar-datos").click(registrarEquipo);
